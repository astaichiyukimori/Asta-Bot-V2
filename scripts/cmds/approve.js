    ...
    switch (type) {
      ...
      case "-a": {
        ...
        if ((args[1].length !== 16) && (args[1].length !== 17)) {
          api.sendMessage("⚠ Thread ID Must Be Exactly 16 or 17 Digit Number.", event.threadID);
          return;
        }
      ...
      case "-fd": {
        try {
          if (!args[1]) {
            api.sendMessage(`⚠ Please Provide The threadID or The Name Of The Data.`, event.threadID);
            return;
          }
          idBox = args[1];
